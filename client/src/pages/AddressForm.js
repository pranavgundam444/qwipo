  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingAddress && editingAddress.id) {
        ...
        await axios.put(`https://qwipo-jjt5.onrender.com/api/addresses/${editingAddress.id}`, form);
      } else {
        // Create new address for customer
        await axios.post(`https://qwipo-jjt5.onrender.com/api/customers/${customerId}/addresses`, form);
      }
      setForm({ address_details: "", city: "", state: "", pin_code: "" });
      setError("");
      if (onSaved) onSaved();
